        ...
        <div class="ias-header">
          ...
          <div class="ias-header-meta">
            ...
            <!-- GET now sits beneath the Title/Subtitle stack, matching the
                 ...
                 does. "In-App Purchases" (when shown) now sits beside GET
...
/* ═══════════════════════════════════════════════════
   ...
   section: a reorderable list of achievements, each with Display Name/
   Earned Description/Pre-Earned Description/Image directly on it (no
   per-language Localizations sub-list — removed by request; see
   state.macGameCenterAchievements' own comment, state.js). Reuses the IAP
   Products list's row/field classes (.iap-product-row, .iap-product-field,
   .iap-product-remove, .iap-product-trial) purely for visual consistency —
   these achievements have nothing to do with IAP.

   Reordering is drag-and-drop on the COLLAPSED row only (app.js's
   macGcAchievementDragStart/DragOver/Drop/DragEnd) — there's no separate
   up/down control any more. Collapsing is available two ways: the existing
   Save button at the bottom of the expanded editor, and — new — clicking
   the header bar at the top of an expanded row (the same
   saveMacGcAchievement call, so it's a no-op until Reference Name is
...
// DragEnd, app.js) — a small ⠿ handle makes the affordance visible. Shows
// the Display Name when one exists, falling back to the internal Reference
// Name so a not-yet-filled-in achievement still has a recognizable label.
function _macGcAchievementCollapsedRow(a) {
  ...
  return `
    <div class="iap-product-row mac-gc-row is-collapsed" data-ach-id="${a.id}" style="display:flex;align-items:center;gap:10px;"
         draggable="true"
         onclick="expandMacGcAchievement('${a.id}')"
         ondragstart="macGcAchievementDragStart(event,'${a.id}')"
         ondragover="macGcAchievementDragOver(event,'${a.id}')"
         ondragleave="macGcAchievementDragLeave(event)"
         ondrop="macGcAchievementDrop(event,'${a.id}')"
...
// Expanded editor — a clickable header bar (click to collapse, via the same
// saveMacGcAchievement Reference-Name-required gate as the Save button),
// then Reference Name/Point Value/Hidden/Achievable More Than Once/Display
// Name/Earned Description/Pre-Earned Description/Image, then the Save
// button.
