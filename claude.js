...
async function igdbSearch(title) {
  ...
  return games.map(g => ({
    ...
    // and pull its library_capsule/library_hero Key Art assets.
    steamAppId: (() => {
...
/* ── SteamDB Key Art lookup ──────────────────────────────────
   ...
   pulls the library_capsule/library_hero asset links out of its Assets
   table. steamdb.info is plain server-rendered HTML (no JS execution
   needed), proxied through corsproxy.io using the same raw pass-through
   form as IGDB_ENDPOINT above (corsproxy.io/?<target-url>, no encoding —
   an earlier, unverified `?url=<encoded>` form was tried and silently
...
// Each asset row looks like: <td>library_capsule</td><td>[<a href="...">]{value}[</a>]</td>
// `value` is sometimes the full absolute CDN URL, sometimes just the bare
// "{asset code}/library_capsule.jpg" path steamdb.info shows next to the
...
// Matches on the exact closing `</td>` after the key so "library_capsule"
// doesn't also match its "library_capsule_2x" sibling row.
...
async function fetchSteamDbKeyArt(appId) {
  ...
  return {
    capsuleUrl: _extractSteamDbAsset(html, 'library_capsule', appId),
